<h1 align="center" style="color: #9b59b6;">🐈‍⬛ • Olá, eu sou a Vitória! Uma desenhista e programadora amante de felinos ’</h1>

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&duration=3000&pause=500&color=C097FF&center=true&vCenter=true&multiline=true&width=435&lines=Programadora+J%C3%BAnior+apaixonada+por+tecnologia;gatos%2C+codificação+e+arte+digital;Criadora+da+Serena+Cat+%E2%9C%A8"/>
</p>

```js
// Perfil resumido
const vitoria = {
  role: "Junior Developer",
  skills: ["JavaScript", "Discord Bots", "Web Design", "Artista Digital"],
  loves: ["Gatos", "Arte", "Criar projetos únicos"]
};
```

## 🐾 Habilidades

- **Linguagens**: JavaScript, HTML  
- **Ferramentas**: Node.js, Forgescript, Git
- **Bibliotecas**: Forgescript & Discord.js  
- **Design**: Ilustrações / Digital Art  

## ✨ Meus Projetos

- **[Serena Cat](https://github.com/VittyhSoffyy12/serena-cat)** – Bot de moderação e diversão pro Discord com um toque fofo e floral.  
- **[Meu Site](https://serenacat.code.blog)** – Um cantinho todo florido diante meus projetos tão fechados.
## 🐈‍⬛ Curiosidades

- Sou uma furry e adoro personagens neste estilo.  
- Meu foco é aprender mais sobre desenvolvimento web e bots para Discord.  
- Acredito que programação também é uma forma de arte.
- Curto pra caramba as cores preto e roxo, por algum motivo.

---
***Se quiser conversar, colaborar ou só dar um oi:***

[![Discord](https://img.shields.io/badge/Discord-vitoria_sophia-7289DA?logo=discord&logoColor=white&style=for-the-badge)](https://discord.com/users/999208637509668954)
[![GitHub](https://img.shields.io/badge/GitHub-VittyhSoffy-000000?style=for-the-badge&logo=github&logoColor=purple)](https://github.com/VittyhSoffyy12)
